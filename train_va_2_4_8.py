#!/usr/bin/env python3
"""
使用 VA（Visual Adapter）训练 InCTRL 的 2/4/8-shot 模型。
...
# 设置文件日志
LOG_FILE = PROJECT_ROOT / "training_va_2_4_8.log"
logging.basicConfig(
    level=logging.INFO,
...
def train_model(
    ...
    """
    logger.info(f"========== 训练实验 (VA, shot={shot}) ==========")
...
if __name__ == "__main__":
    logger.info("=" * 72)
    logger.info("VA 2/4/8-shot 训练开始")
    logger.info("=" * 72)
    ...
    for shot in TARGET_SHOTS:
        ...
        logger.info(
            f"严格等算力模式: shot={shot}, batch_size={BATCH_SIZE} -> {effective_batch_size}"
        )
    ...
    logger.info("  - checkpoints/InCTRL_trained_on_MVTec_VA/2/checkpoint")
