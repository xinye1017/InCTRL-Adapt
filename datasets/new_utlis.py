...
class BalancedBatchSampler(Sampler):
    def __init__(self, cfg, dataset):
        ...
        # 直接从 dataset 的 sample 列表读取标签，避免触发 __getitem__（每张图片的 PIL.open）
        label_list = []
        for sample in dataset.normal_samples:
            label_list.append(int(sample['target']))
        for sample in dataset.outlier_samples:
            label_list.append(int(sample['target']))
