...
from open_clip.inctrl_pqa import (
    InCTRLPQA,
    _fuse_maps,
    _fuse_scores,
    _get_vision_width,
...
def test_compute_patch_residual_returns_zero_for_identical_query_and_prompt_tokens():
    ...
    residual = InCTRLPQA._compute_patch_residual(model, query, prompts)
...
def test_upsample_patch_map_expands_square_patch_grid_to_image_size():
    ...
    upsampled = InCTRLPQA._upsample_patch_map(model, patch_map)
...
def test_disabled_text_branch_outputs_neutral_zero_logits_without_tokenizer():
    ...
    outputs = InCTRLPQA._zero_text_outputs(model, query_global, patch_tokens)
...
def test_disabled_pqa_branch_outputs_neutral_zero_logits_and_map():
    ...
    outputs = InCTRLPQA._zero_pqa_outputs(model, query_tokens)
...
def test_disabled_pqa_branch_contributes_zero_final_map_component():
    ...
    pqa_map = InCTRLPQA._pqa_map_from_logits(model, logits)
